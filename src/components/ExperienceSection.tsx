...
export const ExperienceSection = () => {
  ...
  const renderExperienceItem = (item: typeof experiences[0]) => (
    <div
      key={item.number}
      className={cn(
        "w-[180px] flex flex-col items-start gap-2 pt-3 border-t border-lightblue-1/50", // Viền trên màu trắng khi không hover
        "cursor-pointer p-4 transition-all duration-300", // Giữ padding, loại bỏ bo góc mặc định
        "hover:bg-darkblue-5/20 hover:border hover:border-lightblue-5 hover:rounded-lg" // Khi hover: nền mờ, viền xanh dương ở 4 cạnh và bo góc
      )}
    >
